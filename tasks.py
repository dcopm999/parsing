...
def coverage(c):
    ...
    """
    c.run("coverage run --source parsing runtests.py tests")
...
def docs(c):
    ...
    """
    c.run("rm -f docs/parsing.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ parsing")
...
def release(c, bumpsize=''):
    ...
    c.run('git tag -a {version} -m "New version: {version}"'.format(version=parsing.__version__))
    c.run("git push --tags")
